	...
	case IDENT:
		...
		if (SYM==BECOMES){
			...
			if (i!=0){ //??????????
                                ...
                                EXPRESSION(FSYS,LEV,TX); //?????????§Õ??????????????
                                GEN(OPR,0,2); //???????????????????
                                GEN(STO,LEV-TABLE[i].vp.LEVEL,TABLE[i].vp.ADR); //?????????????????????§Ò???
                        }
		}
	...
	case IFSYM:
		...
		STATEMENT(FSYS,LEV,TX);  /*????then????????*/
                if(SYM!=ELSESYM){CODE[CX1].A=CX;} /*?????if then??? ?????????????????*/
                        ...
                        CX2=CX;
                        GEN(JMP,0,0);  /*??????????????????????????????else????????????¦Â???*/
                        CODE[CX1].A=CX; /*CX1????????????????if?false???????????????????????????jump?????????????jump???????§µ???*/
                        STATEMENT(FSYS,LEV,TX); /*????else????*/
                        CODE[CX2].A=CX; /*CX2?????????????????????????if?true???????§Ö?jump???????????????else???????§µ?*/
                        }
		break;
	case BEGINSYM:
		GetSym();
       ...
       case FORSYM:
		...
		if (i==0) {
			...
			Error(12);
                ...
                STATEMENT(SymSetUnion(SymSetNew(TOSYM,DOWNTOSYM),FSYS),LEV,TX);//?????????????TO????DOWNTO??¦Â
                if(SYM==TOSYM){//?§Ø???????????TO
                        GetSym();//???TO?????????
                        CX1=CX;//????????????????????§Ø?¦Ë?????
                        GEN(LOD,LEV-TABLE[i].vp.LEVEL,TABLE[i].vp.ADR);//????????????A????????
                        EXPRESSION(SymSetUnion(SymSetNew(TOSYM,DOWNTOSYM,DOSYM),FSYS),LEV,TX);//??TO????????????§Õ??????????????????????
                        GEN(OPR,0,10);//??????????§³???????A<B????AB?????????????
                        if(SYM==DOSYM){//?§Ø???????????DO
                                CX2=CX;//???DO??§á????????
                                GEN(JPC,0,0);//???????????
                                ...
                                GEN(OPR,0,2);//????????????????????????????A+1??
                                GEN(STO,LEV-TABLE[i].vp.LEVEL,TABLE[i].vp.ADR);//???A????????A++??????
                                GEN(JMP,0,CX1);//???????????AB???????????AB??¦Ä??????????
                                CODE[CX2].A=CX;//???JPC??????????????JPC?§Ø????????????????
                        }else Error(18);//???DO????????
                }
                else if(SYM==DOWNTOSYM){//?§Ø???????????DOWNTO
                        GetSym();//???DOWNTO?????????
                        CX1=CX;//????????????????????§Ø?¦Ë?????
                        EXPRESSION(SymSetUnion(SymSetNew(TOSYM,DOWNTOSYM,DOSYM),FSYS),LEV,TX);//??DOWNTO????????????§Õ??????????????????????
                        if(SYM==DOSYM){//?§Ø???????????DO
                                GEN(LOD,LEV-TABLE[i].vp.LEVEL,TABLE[i].vp.ADR);//????????????A????????
                                GEN(OPR,0,10);//??????????§³???????A<B????AB?????????????
                                CX2=CX;//???DO??§á????????
